...
def test_should_create_vache_given_valid_state():
    # Arrange
    vache = Vache(petitNom="Marguerite", poids=450.0, panse=10.0, age=5)
...
def test_should_raise_invalid_vache_exception_given_empty_petit_nom(petitNom):
    ...
    with pytest.raises(InvalidVacheException):
        Vache(petitNom=petitNom, poids=450.0, panse=10.0, age=5)
...
def test_should_raise_invalid_vache_exception_given_invalid_age(age):
    # Arrange / Act / Assert
    with pytest.raises(InvalidVacheException):
        Vache(petitNom="Marguerite", poids=450.0, panse=10.0, age=age)
...
def test_should_raise_invalid_vache_exception_given_negative_poids_or_panse(poids, panse):
    # Arrange / Act / Assert
    with pytest.raises(InvalidVacheException):
        Vache(petitNom="Marguerite", poids=poids, panse=panse, age=5)
...
def test_should_raise_invalid_vache_exception_given_typed_food_when_brouter_on_vache():
    ...
    # Act / Assert
    with pytest.raises(InvalidVacheException):
        vache.brouter(5.0, nourriture="HERBE")  # n'importe quoi ≠ None doit être refusé


def test_should_allow_brouter_given_quantity_that_reaches_panse_max_exactly():
    # Arrange
    vache = Vache(petitNom="Marguerite", poids=450.0, panse=Vache.PANSE_MAX - 5.0, age=5)
...
def test_should_raise_invalid_vache_exception_given_quantity_that_exceeds_panse_max_when_brouter():
    ...
    # Act / Assert
    with pytest.raises(InvalidVacheException):
        vache.brouter(5.0001)  # dépasse PANSE_MAX


@pytest.mark.parametrize(
    "panse_initiale, quantite",
    [
        (Vache.PANSE_MAX, 0.1),     # déjà pleine, toute quantité positive dépasse
        (Vache.PANSE_MAX - 0.01, 0.02),  # dépassement léger
        (Vache.PANSE_MAX - 1.0, 2.0),    # dépassement net
    ]
)
def test_should_raise_invalid_vache_exception_given_panse_overflow_cases_when_brouter(panse_initiale, quantite):
    # Arrange
    vache = Vache(petitNom="Marguerite", poids=450.0, panse=panse_initiale, age=5)
...
def test_should_increase_poids_given_positive_panse_when_ruminer():
    # Arrange
    vache = Vache(petitNom="Marguerite", poids=450.0, panse=20.0, age=5)
...
def test_should_raise_invalid_vache_exception_given_empty_panse_when_ruminer():
    # Arrange
    vache = Vache(petitNom="Marguerite", poids=450.0, panse=0.0, age=5)
...
def test_should_raise_invalid_vache_exception_given_age_max_when_vieillir():
    # Arrange
    vache = Vache(petitNom="Marguerite", poids=450.0, panse=10.0, age=Vache.AGE_MAX)
